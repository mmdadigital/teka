...
// Add Zen Tabs styles
if (theme_get_setting('teka_tabs')) {
  drupal_add_css( drupal_get_path('theme', 'teka') .'/css/tabs.css');
}
...
function teka_preprocess_html(&$vars) {
  ...
  foreach ($user->roles as $role){
    $vars['classes_array'][] = 'role-' . teka_id_safe($role);
  }
  ...
  if (!$vars['is_front']) {
    ...
    $vars['classes_array'][] = teka_id_safe('page-'. $path);
    $vars['classes_array'][] = teka_id_safe('section-'. $section);
  ...
  // Add IE classes.
  if (theme_get_setting('teka_ie_enabled')) {
    $teka_ie_enabled_versions = theme_get_setting('teka_ie_enabled_versions');
    if (in_array('ie8', $teka_ie_enabled_versions, TRUE)) {
...
function teka_preprocess_node(&$vars) {
  ...
  // Merge first/last class (from teka_preprocess_page) into classes array of current node object.
  $node = $vars['node'];
...
function teka_preprocess_block(&$vars, $hook) {
  // Add a striping class.
...
function teka_breadcrumb($variables) {
  ...
  $show_breadcrumb = theme_get_setting('teka_breadcrumb');
  if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {

    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('teka_breadcrumb_home');
    ...
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = theme_get_setting('teka_breadcrumb_separator');
      $trailing_separator = $title = '';
      if (theme_get_setting('teka_breadcrumb_title')) {
        $item = menu_get_item();
      ...
      elseif (theme_get_setting('teka_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }
 ...
 *
 */
function teka_menu_link(array $variables) {
  ...
  $element['#attributes']['class'][] = teka_id_safe($element['#title']);
 ...
 */
function teka_preprocess_menu_local_task(&$variables) {
 ...
 */
function teka_menu_local_tasks(&$variables) {
  $output = '';
